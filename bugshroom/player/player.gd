...
@onready var state_machine_playback : AnimationNodeStateMachinePlayback = animation_tree.get("parameters/BaseStateMachine/playback")
...
# handle attack
	...
	if Input.is_action_pressed("sprint_%s" % [player_id]) and current_stamina > 0:
		...
		stamina_bar.update()
		speed = SPRINT_SPEED
		#animation_tree.set("parameters/BaseStateMachine/movement/blend_position", 2)
	...
	if not is_rooted and !is_dead:
		if direction:
			...
			if is_on_floor() and !is_rooted and !is_dead:
				pass
				#animation_tree.set("parameters/BaseStateMachine/movement/blend_position", 1)
		...
		velocity.x = 0
		velocity.z = 0 
		#animation_tree.set("parameters/BaseStateMachine/movement/blend_position", 0)
...
func take_damage(amount):
	animation_tree["parameters/OneShotTakeDamage/request"] = AnimationNodeOneShot.ONE_SHOT_REQUEST_FIRE
...
func toggle_root():
	...
	if is_rooted:
		...
		state_machine_playback.travel("uncrouch")
		#animation_tree["parameters/Oneshot_Crouch/request"] = AnimationNodeOneShot.ONE_SHOT_REQUEST_FIRE
...
func die():
	...
	state_machine_playback.travel("die")
